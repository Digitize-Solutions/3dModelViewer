...
export default class FWDEMVUtils{
	

	/**
	 * 3D/
	 */
   	static getClosestBone(skinnedMesh, vertexIndex, model, boneName){

		let bone;

		// Get bone from skined mesh and vertext index.
		if(skinnedMesh && vertexIndex){
			let vi = vertexIndex;
		...
		// Get bone based on name from a model.
		if(model && boneName){
	...
	static getMeshByName(model, meshName) {
		let foundMesh = null;
	...
	static areVectorsEqual(vector1, vector2, decimalPlaces) {
		var tolerance = Math.pow(10, -decimalPlaces);
    ...
    static getChildrenFromAttribute(el, attr, allNodesTypes){
		let kids = [];
		for(let c = el.firstChild; c != null; c = c.nextSibling){
			if(allNodesTypes && FWDEMVUtils.hasAttribute(c, attr)){
				kids.push(c);
			}else if(c.nodeType === 1 && FWDEMVUtils.hasAttribute(c, attr)){
				kids.push(c);
			}
		}
    ...
    static getChildAt(el, index){
		let kids = FWDEMVUtils.getChildren(el);
    ...
    static getAttributeValue = function(el, attr){
		if(!FWDEMVUtils.hasAttribute(el, attr)) return undefined;
		return el.getAttribute(attr);	
	};
	
	static insertNodeAt = function(prt, child, node){
		let children = FWDEMVUtils.children(prt);
	...
	static getViewportSize(){
		if(FWDEMVUtils.hasPointerEvent && navigator.msMaxTouchPoints > 1){
			return {w:document.documentElement.clientWidth || window.innerWidth, h:document.documentElement.clientHeight || window.innerHeight};
		}
		
		if(FWDEMVUtils.isMobile) return {w:window.innerWidth, h:window.innerHeight};
		return {w:document.documentElement.clientWidth || window.innerWidth, h:document.documentElement.clientHeight || window.innerHeight};
	};
	
	static getViewportMouseCoordinates(e){
		let offsets = FWDEMVUtils.getScrollOffsets();
	...
	static getValidSource =  function(source){
		...
		if(!FWDEMVUtils.isURLEncoded(firstUrlPath)){
		...
		if(source.match(/\.mp3|\.mp4|\.m3u8|\.txt|\.srt|\.vtt|\.jpg|\.jpeg|\.png/ig)
			...
			if(FWDEMVUtils.isURLEncoded(secondUrlPath)){
	...
	static setMediaType(src, obj){
		if(/\.jpg|\.webp|\.jpeg|\.png|\.bmp|\.gif|\.tif|\.tiff|\.jfi|\.jfif|\.exif|\.svg/i.test(src)){
			obj.type = FWDEMVUtils.IMAGE;
		}else if(/\.mp4|\.m3u8|\.mpd/i.test(src)){
			obj.type = FWDEMVUtils.VIDEO;
		}
	};
	...
	static splitAndTrim(str, trim, toLowerCase){
		var array = str.split(",");
		var length = array.length;
		for(var i=0; i<length; i++){
			if(trim) array[i] = array[i].trim()
			if(toLowerCase) array[i] = array[i].toLowerCase();
		};
		return array;
	};


	// Get object from array based on name
	static getObjectFromArrayByName(array, name) {
		let foundTexture = array.find(function (data) {
			return data.name === name;
		});
	
		return foundTexture || null; // Return null if not found
	}
	...
	static getRGBA(colorString) {
		...
		if (rgbaMatch) {
			...
			const color = '#' + FWDEMVUtils.rgbToHex(r, g, b);
		
			return { color: color, alpha: a };
		} else {
	...
	// Traverse materials.
	static traverseMaterials (object, callback){
